...
```
In order to check the statistical significance of the computed correlation, let's do a permutation analysis. Taking `r N` permutations, we find the p-value of the measured correlation to be `r p_value_Cor`. As the p-value is greater than 0.05, the measured correlation is not statistically significant.
...
Let's carry out a mediation analysis to check if the correlation with the GDP per capita of the total unemployment is mediated by unemployment in one of population sub-groups.
